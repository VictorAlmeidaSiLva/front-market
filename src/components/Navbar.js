...
const Navbar = ({ logo }) => {
  ...
  return (
    <div className="flex flex-row items-center justify-between h-28" style={{ backgroundColor: 'black' }}>
      ...
      {openNav && (
        <div ref={wrapperRef} className="absolute top-0 w-full h-48 bg-white transition-transform transform translate-y-0 duration-300 ease-in-out">
          <div onClick={() => setOpenNav(false)} className="absolute right-12 top-9 cursor-pointer">
            <img height="30" width="30" src={close_icon} alt="Close" />
          </div>
